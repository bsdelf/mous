...
inline bool CreateFile(const char* path, uint64_t size)
{
    using namespace std;
...
inline std::string FileSuffix(const std::string& name, char ch = '.')
{
    size_t pos = name.find_last_of(ch);
    return name.substr(pos+1, name.size());
}

inline std::string FileDir(const std::string& path)
{
    using namespace std;
...
inline std::string ReadAll(const char* filename, bool asBin = false)
{
    using namespace std;
