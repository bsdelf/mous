...
public:
    ...
    void PrintMS() const
	{
        std::cout << (time_t)DiffMS() << std::endl;
    }

    void PrintUS() const
	{
        std::cout << (time_t)DiffUS() << std::endl;
    }
    ...
    double DiffUS() const
	{
		return (m_End.tv_sec - m_Begin.tv_sec)*1000000.f + (m_End.tv_nsec - m_Begin.tv_nsec)/1000.f;
    }

    double DiffCurrentMS() const
    {
        return DiffCurrentUS() / 1000.f;
    }
