...
#define SCX_COPY_SOCKETCOMMON(Socket)                                                                                          \
  public:                                                                                                                      \
    Socket(const Socket& socket) { m_Fd = dup(socket.m_Fd); }                                                                  \
                                                                                                                               \
    ~Socket() { Close(); }                                                                                                     \
                                                                                                                               \
    ...
    int Fd() const { return m_Fd; }                                                                                            \
                                                                                                                               \
    InetAddr& Addr() { return m_InetAddr; }                                                                                    \
                                                                                                                               \
    const InetAddr& Addr() const { return m_InetAddr; }                                                                        \
                                                                                                                               \
...
class TcpSocket
{
  ...
  public:
    TcpSocket() { m_Fd = socket(AF_INET, SOCK_STREAM, 0); }

    bool Listen(int backlog = -1) const { return listen(m_Fd, backlog) == 0; }
    ...
    bool Connect(const InetAddr& iadr) const { return connect(m_Fd, (struct sockaddr*)&iadr.m_Addr, iadr.m_AddrLen) == 0; }

    ssize_t Recv(void* buf, size_t len, int flags = 0) const { return recv(m_Fd, buf, len, flags); }

    ssize_t Send(const void* buf, size_t len, int flags = 0) const { return send(m_Fd, buf, len, flags); }
