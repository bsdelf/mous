...
public:\
	...
	m_RefMap.insert(std::pair<FunctionBase_t*, size_t>(m_PtrFn, 1));\
    }									\
    \
...
public:\
    ...
    \
    Function(const Function& f): m_PtrFn(f.m_PtrFn) {	\
	if (m_PtrFn != NULL)				\
	    ++m_RefMap[m_PtrFn];			\
    }							\
    \
...
private:\
    void CheckRefMap() {						\
	if (m_PtrFn != NULL) {						\
	    typename std::map<FunctionBase_t*, size_t>::iterator iter = \
