...
class LPVBuffer
{
  public:
     ...
     */
    item_t* RawItemAt(size_t i) { return m_BufferArray + i; }
    ...
    void RecycleFree(item_t* val = nullptr) { m_DataListSemVar.Post(); }
    ...
    void RecycleData(item_t* p = nullptr) { m_FreeListSemVar.Post(); }

    /**
     * Clear the empty-buffer, so that the producer can be blocked.
     */
    void ClearFree() { m_FreeListSemVar.Clear(); }

    /**
     * Clear the filled-data, so that the customer can be blocked.
     */
    void ClearData() { m_DataListSemVar.Clear(); }
