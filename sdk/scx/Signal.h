 ...
 private:
  ...
  struct FunctionSlot : public Slot {
    FunctionSlot(const F& fn)
        : Slot(SlotType::Function), fn(fn) {
    }

    FunctionSlot(F&& fn)
        : Slot(SlotType::Function), fn(std::move(fn)) {
    }

    R Invoke(P... p) const final {
      return fn(p...);
    }
  ...
  struct RawFunctionSlot : public Slot {
    explicit RawFunctionSlot(RawFunction fn)
        : Slot(SlotType::RawFunction), fn(fn) {
    }
  ...
  struct MemberFunctionSlot : public Slot {
    explicit MemberFunctionSlot(O* obj)
        : Slot(SlotType::MemberFunction), obj(obj) {
    }
  ...
  struct SignedMemberFunctionSlot : public MemberFunctionSlot<O> {
    SignedMemberFunctionSlot(F fn, O* obj)
        : MemberFunctionSlot<O>(obj), fn(fn) {
    }

    R Invoke(P... p) const final {
      return (this->obj->*fn)(p...);
    }

    F const fn;
  };

  static auto MakeSlot(RawFunction f) {
    return std::make_unique<RawFunctionSlot>(f);
  }
