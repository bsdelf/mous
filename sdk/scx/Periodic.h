  ...
  class Task {
   public:
    ...
    Task(F&& callback, int interval, bool oneshot)
        : callback_(std::forward<F>(callback)), interval_(interval), oneshot_(oneshot), deadline_(std::chrono::steady_clock::now() + interval_) {
    }
    ...
    auto Deadline() const -> const std::chrono::steady_clock::time_point& {
      return deadline_;
    }
