...
class Playlist {
 ...
 public:
  ...
  void Move(Array oldPos, int newPos) {
    ...
    // calc new seq index
    int seqIndex = (m_Mode == PlaylistMode::Shuffle || m_Mode == PlaylistMode::ShuffleRepeat) ? m_SeqShuffleQueue[m_SeqIndex] : m_SeqIndex;
    ...
    m_SeqIndex = (m_Mode == PlaylistMode::Shuffle || m_Mode == PlaylistMode::ShuffleRepeat) ? MOUS_FIND(m_SeqShuffleQueue, seqIndex) - m_SeqShuffleQueue.begin() : seqIndex;
  }
