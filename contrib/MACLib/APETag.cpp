...
int CAPETagField::SaveField(char * pBuffer)
{
    ...
    *((int *) pBuffer) = m_nFieldFlags;
    ...
    CSmartPtr<char> spFieldNameANSI((char *) m_spFieldNameUTF16, TRUE); 
...
/*
int CAPETag::GetFieldString(const char * pFieldName, str_ansi * pBuffer, int * pBufferCharacters, BOOL bUTF8Encode)
{
    ...
    {
        CSmartPtr<str_ansi> spANSI(bUTF8Encode ? (str_ansi *) pUTF16 : CAPECharacterHelper::GetANSIFromUTF16(pUTF16), TRUE);
    ...
    if (*pBufferCharacters > 0)
    {
        ...
        else if (pAPETagField->GetIsUTF8Text() || (m_nAPETagVersion < 2000))
        {
            ...
            if (m_nAPETagVersion >= 2000)
                spUTF16.Assign((char *) pAPETagField->GetFieldValue(), TRUE);
            else
                spUTF16.Assign(const_cast<char*>(pAPETagField->GetFieldValue()), TRUE);
...
int CAPETag::CreateID3Tag(ID3_TAG * pID3Tag)
{
    ...
    {
        if (strcmp(cBuffer, g_ID3Genre[nGenreIndex]) == 0)
        {
...
int CAPETag::SetFieldString(const char * pFieldName, const char * pFieldValue)
{
    ...
    CSmartPtr<char> spFieldValueUTF8((char *) pFieldValue, TRUE);
...
int CAPETag::SetFieldString(const char * pFieldName, const char * pFieldValue, BOOL bAlreadyUTF8Encoded)
{
    ...
    {
        CSmartPtr<char> spUTF8((char *) pFieldValue, TRUE);
...
int CAPETag::GetFieldID3String(const char * pFieldName, char * pBuffer, int nBytes)
{
    ...
    CSmartPtr<str_ansi> spBufferANSI(cBuffer, TRUE);
