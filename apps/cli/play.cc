...
int cmd_play(int argc, char* argv[]) {
  ...
  for (int ch = -1; (ch = getopt(argc, argv, "rs")) != -1;) {
    switch (ch) {
      case 'r':
        ctx->playlist.SetMode(ctx->playlist.Mode() != PlaylistMode::Shuffle ? PlaylistMode::Repeat : PlaylistMode::ShuffleRepeat);
        break;

      case 's':
        ctx->playlist.SetMode(ctx->playlist.Mode() != PlaylistMode::Repeat ? PlaylistMode::Shuffle : PlaylistMode::ShuffleRepeat);
        break;
