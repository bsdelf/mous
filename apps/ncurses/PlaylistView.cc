...
void PlaylistView::Refresh() {
  ...
  if (!m_List.empty()) {
    ...
    for (int l = 0; l < lcount; ++l) {
      ...
      const string& field1 = MBStrWidth(item.tag.title) <= wField1 - 1 ? item.tag.title : MBWidthStr(item.tag.title, wField1 - 1 - 3) + "...";
      d.Draw(xoff, yoff + l, Text(field1).SetAttributes(fieldAttr).SetColor(fieldColorF, fieldColorB));
      xoff += wField1;

      const string& field2 = MBStrWidth(item.tag.artist) <= wField2 - 1 ? item.tag.artist : MBWidthStr(item.tag.artist, wField2 - 1 - 3) + "...";
      d.Draw(xoff, yoff + l, Text(field2).SetAttributes(fieldAttr).SetColor(fieldColorF, fieldColorB));
      xoff += wField2;

      const string& field3 = MBStrWidth(item.tag.album) <= wField3 - 1 ? item.tag.album : MBWidthStr(item.tag.album, wField3 - 1 - 3) + "...";
